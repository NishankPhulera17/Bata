...
const EditProfile = ({ navigation, route }) => {
  ...
  const [isGstinEntered, setIsGstinEntered] = useState(false);
  ...
  const gstVerified = useSelector((state)=>state.kycDataSlice.gstCompleted)
  ...
  const handleData = (data, title, jsonData) => {
    ...
    if (title == "gstin") {
      setIsGstinEntered(data.length > 0);
    }
        ...
        {!isClicked && (
          ...
          >
            <TouchableOpacity
              ...
              disabled={!gstVerified && isGstinEntered}
            >
              <PoppinsTextMedium
