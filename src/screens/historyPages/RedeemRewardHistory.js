...
import { useCheckSalesBoosterHistoryMutation } from "../../apiServices/salesBooster/salesBoosterApi";
...
const RedeemRewardHistory = ({ navigation }) => {
  ...
  const [mergedSalesBooster, setMergedSalesBooster] = useState();
  ...
  useEffect(() => {
    if (
      workflowProgram.includes("Static Coupon") ||
      workflowProgram.includes("static coupon")
    ) {
      setShowCoupons(true);
    }
    if (
      workflowProgram.includes("Wheel") ||
      workflowProgram.includes("wheel")
    ) {
      setShowWheel(true);
    }
    if (
      workflowProgram.includes("Points On Product") ||
      workflowProgram.includes("points on product")
    ) {
      setShowPoints(true);
    }
    if (
      workflowProgram.includes("Cashback") ||
      workflowProgram.includes("cashback")
    ) {
...
//   ] = useGetAllRedeemedCouponsMutation();
  ...
  const [
    getSalesBoosterHistoryFunc,
    {
      data: getSalesBoosterHistoryData,
      error: getSalesBoosterHistoryError,
      isLoading: getSalesBoosterHistoryIsLoading,
      isError: getSalesBoosterHistoryIsError,
    },
  ] = useCheckSalesBoosterHistoryMutation();

  const [
    checkSalesBoosterOnEachScanFunc,
    {
  ...
  useEffect(() => {
    const getToken = async () => {
      ...
      if (credentials) {
        ...
        getSalesBoosterFunc(token);
        // checkSalesBoosterOnEachScanFunc(token);
      }
    };
    getToken();
  }, [focused]);
...
//   }, [getAllRedeemedError, getAllRedeemedData]);
  ...
  useEffect(() => {
    if (fetchUserPointsHistoryData) {
      console.log(
        ...
        fetchUserPointsHistoryData.body.data
      );
  ...
  useEffect(() => {
    if (fetchCashbackEnteriesData) {
      console.log(
        ...
        fetchCashbackEnteriesData.body.data
      );
  ...
  useEffect(() => {
    if (getSalesBoosterData) {
      ...
      if(getSalesBoosterData.status==200)
      {
        const getToken = async () => {
          const credentials = await Keychain.getGenericPassword();
          if (credentials) {
            ...
            const token = credentials.username;
            getSalesBoosterHistoryFunc(token);
            // checkSalesBoosterOnEachScanFunc(token);
          }
        };
        getToken();
      ...
      if (
        getSalesBoosterError?.data?.message == "Invalid JWT" &&
        getSalesBoosterError?.status == 401
      ) {
        removerTokenData();
      }
  ...
  useEffect(() => {
    if (getSalesBoosterHistoryData) {
      ...
      if (
        getSalesBoosterHistoryError?.data?.message == "Invalid JWT" &&
        getSalesBoosterHistoryError?.status == 401
      ) {
        removerTokenData();
      }
    }
  ...
  // useEffect(() => {
  //   if (
  //     getSalesBoosterData?.status == 200 &&
  //     checkSalesBoosterOnEachScanData?.status == 200
  //   )
  //     setMergedSalesBooster(
  //       mergeNestedArrays(
  //         getSalesBoosterData?.body,
  //         checkSalesBoosterOnEachScanData?.body
  //       )
  //     );
  // }, [getSalesBoosterData, checkSalesBoosterOnEachScanData]);
  ...
  const mergeNestedArrays = (arr1, arr2) => {
    ...
    for (let i = 0; i < maxLength; i++) {
      // Merge corresponding inner arrays or use an empty array if one doesn't exist
      const mergedInnerArray = (arr1[i] || []).concat(arr2[i] || []);
      merged.push(mergedInnerArray);
    }
  ...
  const DreamGiftComponent = () => {
    const milestoneData = {
      campaign: null,
      data: [
        "https://paramount-dev.genefied.co/uploads/dashboard-banners/1692253456.3752-134428.jpg",
        "https://paramount-dev.genefied.co/uploads/dashboard-banners/1692170724.6659-716345.jpg",
        "https://paramount-dev.genefied.co/uploads/dashboard-banners/1692168073.1434-51908.jpg",
        "https://paramount-dev.genefied.co/uploads/dashboard-banners/1692167976.6234-134428.jpg",
        "https://paramount-dev.genefied.co/uploads/dashboard-banners/1691221267.6337-63217.jpg",
        "https://paramount-dev.genefied.co/uploads/dashboard-banners/banner-small.png",
      ],
      ...
      milestones: [
        {
          achieved: "1",
          image:
            "https://www.shreesaiholidays.in/wp-content/uploads/2018/01/Goa.png",
          name: "Goa (Pkg for 1) (3N/4D)",
          threshold: "1000",
        },
        {
          achieved: "0",
          image:
            "https://www.shreesaiholidays.in/wp-content/uploads/2018/01/Goa.png",
          name: "Goa (Pkg for 2) (3N/4D)",
          threshold: "1500",
        },
        {
          achieved: "0",
          image:
            "https://i.pinimg.com/originals/9d/54/91/9d5491d5c9b42b9f097af7bd014ac083.jpg",
          name: "Bangkok (Pkg for 2) (3N/4D)",
          threshold: "2200",
        },
        {
          achieved: "0",
          image:
            "https://traveldivaishnavi.com/wp-content/uploads/2018/02/Dubai-5D4N-Package-1-675x460.jpg",
          name: "Dubai (Pkg for 2) (3N/4D)",
          threshold: "3000",
        },
      ],
      pointData: { amount_redeemed: 0, qr_count: 0 },
      status: "200",
      unread_notification_count: "2",
      user: {
        aadhaar: "393856466133",
        aadhaar_image: null,
        aadhaar_verified: "1",
        address:
          "MCF 1753, Gali-43, Faridabad Sector 22, Faridabad Sector 22, Faridabad, Faridabad, Haryana, India, 121005",
        address_2: null,
        alt_mobile: null,
        auth_token: "TdAbU4qyw7ZxJEgFkHfsc35WaVD0u1jSzOo2R8CXQKhvNrPnmp",
        city: null,
        code: "RT00001",
        created_on: "2023-09-22 11:29:00",
        district: null,
        dob: null,
        email: "mitesh.kumar@genuinemark.org",
        est_date: null,
        fcm_token: "fcmtokeforretailer",
        gender: null,
        gst_image: null,
        gstin: "08AAACP0969Q1ZS",
        id: "1",
        ...
        remarks: null,
        shop_image: null,
        shop_name: "PARAMOUNT COMMUNICATIONS LIMITED",
        state: null,
        status: "4",
        updated_on: "2023-09-22 12:04:34",
        user_type: "2",
      },
    };
    return (
      ...
      >
        <View style={{ alignItems: "center", justifyContent: "center" }}>
          <Image
            ...
            source={require("../../../assets/images/cashbackPoints.png")}
          ></Image>
        </View>
        ...
        {fetchCashbackEnteriesData && (
          <FlatList
            ...
            renderItem={({ item, index }) => {
              ...
              return (
                <RedeemRewardDataBoxWithoutImage
                  header="Credited To Balance"
                  data={moment(item.created_at).format("DD MMM YYYY")}
                  points={item.cashback}
                ></RedeemRewardDataBoxWithoutImage>
              );
            }}
      ...
      >
        <View style={{ alignItems: "center", justifyContent: "center" }}>
          <Image
            ...
            source={require("../../../assets/images/magicBox.png")}
          ></Image>
        </View>
        ...
        {fetchGiftsRedemptionsOfUserData && (
          <FlatList
            ...
            renderItem={({ item, index }) => {
              console.log("Redeem", item);
              return (
                <RedeemRewardDataBoxLong
                  type="Uri"
                  header={item.gift.gift[0].name}
                  data={moment(item.created_at).format("DD MMM YYYY")}
                  image={item.gift.gift[0].images[0]}
                ></RedeemRewardDataBoxLong>
              );
            }}
            keyExtractor={(item) => item.id}
    ...
    const Coupon = (props) => {
      ...
      return (
        <ImageBackground
          ...
          source={require("../../../assets/images/voucherBackground.png")}
        >
          <PoppinsTextMedium
            style={{
              color: "#FB774F",
          ...
          >
            ...
            >
              ...
              >
                <PoppinsTextMedium
                  style={{ color: "#DDDDDD", fontWeight: "700", fontSize: 12 }}
                  content={`Code : ${couponCode}`}
                ></PoppinsTextMedium>
              </View>
            </View>
          </View>
        ...
        {/* {getAllRedeemedData && (
          <FlatList
            ...
            renderItem={({ item, index }) => {
              ...
              return (
                <Coupon
                  earnedOn={moment(item.created_at).format("DD MMM YYYY")}
                  couponName={item.brand}
                  expiresOn={moment(item.expire_date).format("DD MMM YYYY")}
        ...
        >
          ...
          >
            ...
            >
              ...
              {getPointSharingCashbackData?.body?.cashback_balance > 0 && (
                <RedeemRewardDataBox
                  navigation={navigation}
                  header="Cashback"
                  data={getPointSharingCashbackData?.body?.cashback_balance}
                  image={require("../../../assets/images/cashback.png")}
                ></RedeemRewardDataBox>
              )}
              {showPoints && userPointData && (
                <RedeemRewardDataBox
                  navigation={navigation}
                  header="Earned Points"
                  data={userPointData.body.point_earned}
              ...
              {showPoints && userPointData && (
                <RedeemRewardDataBox
                  ...
                  data={userPointData.body.point_balance}
                  image={require("../../../assets/images/points.png")}
                ></RedeemRewardDataBox>
              )}
              ...
              {showPoints && userPointData && (
                <RedeemRewardDataBox
                  ...
                  data={userPointData.body.point_redeemed}
                  image={require("../../../assets/images/points.png")}
                ></RedeemRewardDataBox>
              )}
